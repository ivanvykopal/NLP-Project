...
class LSTMClassifier2(torch.nn.Module):
    def __init__(
            self, 
            embedding_dim=100, 
            hidden_dim=64, 
        ...
        self.lstm = torch.nn.LSTM(embedding_dim, hidden_dim, num_layers=n_layers, bidirectional=bidirectional)
        # softmax layer
        self.fc1 = torch.nn.Linear(hidden_dim * self.input_size_factor, output_dim)
        # activation ReLU
    ...
    def forward(self, inputs, return_activations=False):
        ...
        outputs_act = self.fc1(activations)
        # outputs = self.act(outputs)
        # outputs_act = self.fc2(outputs)
        outputs = self.softmax(outputs_act)
